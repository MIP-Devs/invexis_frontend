...
export default async function OverviewPage() {
    ...
    if (session?.accessToken) {
        ...
        await Promise.all([
            ...
            queryClient.prefetchQuery({
                queryKey: ['overview', 'stock-changes', companyId],
                queryFn: () => OverviewService.getStockChanges({ companyId, limit: 50 }, options),
            }),
            queryClient.prefetchQuery({
                queryKey: ['overview', 'products', companyId],
                queryFn: () => OverviewService.getProducts({ companyId, limit: 50 }, options),
            }),
            queryClient.prefetchQuery({
            ...
            queryClient.prefetchQuery({
                ...
                queryFn: () => OverviewService.getInventoryTrends({ companyId, period: 'month' }, options),
            }),
        ]);
    }

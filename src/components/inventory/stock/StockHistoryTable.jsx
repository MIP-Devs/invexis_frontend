...
import { ArrowDownCircle, ArrowUpCircle, History, RefreshCw, Search, Filter } from "lucide-react";
...
export default function StockHistoryTable({ companyId }) {
    ...
    const filteredChanges = changes.filter(change => {
        const matchesSearch = searchTerm === "" ||
            (change.product?.name || "").toLowerCase().includes(searchTerm.toLowerCase()) ||
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
            ...
            <div className="p-6 border-b border-gray-100">
                <div className="flex items-center gap-3 mb-4">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-gray-500">View all stock changes and adjustments</p>
                    </div>
                </div>
            ...
            {loading ? (
                ...
                <div className="p-12 text-center">
                    ...
                    <button onClick={fetchChanges} className="mt-2 text-orange-600 hover:underline">
                        Try again
                    </button>
                </div>
                        ...
                        <TableBody>
                            {filteredChanges.map((change, index) => (
                                <TableRow key={change._id || index} hover>
                                    <TableCell>
                                        <div className={`inline-flex items-center gap-1.5 px-2 py-1 rounded-full text-xs font-medium ${change.type === "in"
                                    ...
                                    <TableCell>
                                        <span className="text-gray-500 text-sm">{formatDate(change.createdAt)}</span>
                                    </TableCell>
                                    <TableCell>
                                        <span className="text-gray-600">{change.user?.name || change.createdBy || "System"}</span>
                                    </TableCell>
                                </TableRow>
                            ))}
                        </TableBody>
